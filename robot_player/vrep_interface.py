...
class VrepInterface(object):
    ...
    def __init__(self, motor_id, dt, robot_name="", gyroscope=False, accelerometer=False, ft_sensor_names=None):
        vrep.simxFinish(-1)
    ...
    def get_joint_data_structure(self, motor_ids):
        ...
        # setup handles
        joint_handle = self.get_joint_handles()
        ...
        # first, organize joint_data by handle
        joint_data_by_handle = {h:{'joint_type':type, 'joint_mode':mode, 'limit_low':limit_low, 'range':joint_range} \
                        for h, type, mode, limit_low, joint_range \
                        in zip(handles, joint_type_list, joint_mode_list, joint_limit_low_list, joint_range_list)}

        # reorganize by joint id
        joint_data = []
        for name, h in joint_handle.items():
            joint_data_by_handle[h].update(
                {'sim_handle': h, 'sim_name':name, 'id':int(name[5+len(self.robot_name):])})
            joint_data.append(joint_data_by_handle[h])

        # filter for specified joints and sort joints to be in order
        # joint_data = [{'sim_handle': joint_handle[name], 'sim_name':name, 'id':int(name[5+len(self.robot_name):])} for name in joint_handle.keys()]
        new_joint = [entry for entry in joint_data if entry['id'] in motor_ids] # filter for specified joints
    ...
    def get_current_position(self, joint):
        joint_angles = [vrep.simxGetJointPosition(self._sim_Client_ID, k['sim_handle'], vrep.simx_opmode_blocking)[1] for k in joint]

        return joint_angles

    def set_command_position(self, joint, commands, send=True):
        for j,c in zip(joint,commands):
            vrep.simxSetJointTargetPosition(self._sim_Client_ID, j['sim_handle'], c,
                                            vrep.simx_opmode_oneshot)
        if send:
            self.send_command()
    ...
    def get_joint_effort(self, joints, send=True):
        effort_list = []
        for j in joints:
    ...
    def get_joint_velocity(self, joint):
        joint_velocity = []
        for j in joint:
            _, vel = vrep.simxGetObjectFloatParameter(self._sim_Client_ID, j['sim_handle'], vrep.sim_jointfloatparam_velocity, \
                                             vrep.simx_opmode_blocking)
            if _ != 0:
    ...
    def set_joint_velocity(self, joint, commands, send=True):
        for j,c in zip(joint,commands):
            vrep.simxGetObjectFloatParameter(self._sim_Client_ID, j['sim_handle'], \
                                             vrep.sim_jointfloatparam_velocity, c, vrep.simx_opmode_oneshot)
        if send:
            self.send_command()
    ...
    def set_all_command_position(self,commands, send_command=True):
        ...
        self.set_command_position(self.joint,commands, send_command)
    ...
    def get_all_current_position(self):
        ...
        return self.get_current_position(self.joint)
    ...
    def get_all_joint_velocity(self):
        ...
        # commands must be for joints ordered from least to greatest
        return self.get_joint_velocity(self.joint)
