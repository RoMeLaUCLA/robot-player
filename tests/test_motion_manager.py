...
class TestNoOptions(unittest.TestCase):
    ...
    def run(self, result=None):
        ...
        with self.assertRaises(ValueError):
            with MotionManager([1, 2], 0.1, 'PLS 2 SEND OPTIONS') as mm:
                self.mm = mm
                super(TestNoOptions, self).run(result)

    def test_nothing(self):
        """
        this is required so that the test class actually executes
        :return:
        """
        pass
...
class TestDxlOptions(unittest.TestCase):
    ...
    def run(self, result=None):
        ...
        """
        motor_ids = [(1, 2), (3, 4)]
        with MotionManager(motor_ids, 0.1, DxlOptions(motor_ids, motor_types=['MX106', 'MX106'], ports=['/dev/ttyUSB2', '/dev/ttyUSB0'], baudrate=3000000, protocol_version=2)) as mm:
            self.mm = mm
